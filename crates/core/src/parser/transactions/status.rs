...
pub fn parse(input: &str) -> IResult<&str, TransactionStatus> {
    context(
        "status",
...
mod test {
    ...
    fn parses_empty_status_as_cleared() {
        assert_eq!(parse(""), Ok(("", TransactionStatus::Cleared)))
    }

    #[test]
    fn ignores_any_other_input_as_cleared() {
        assert_eq!(parse("Test of ignored input"), Ok(("Test of ignored input", TransactionStatus::Cleared)))
    }
